...
type Project struct {
	...
	Tags             utils.Set[tags.Tag]
	size             projectSize // TODO: PROJECT SIZE. Maybe get rid of?
}

type projectSize string

const (
	projectSizeS projectSize = "small"
	projectSizeM projectSize = "medium"
	projectSizeL projectSize = "large"
)
...
func (pg *Project) Dst() string {
	return dstFor("cv", "project", withoutSpaces(pg.Name))
}

func (pg *Project) Title() string {
	if pg == nil {
		return noLinkText
	}
	return pg.Name
...
func (pg *Project) Bytes() []byte {
	...
	if len(pg.CloudProviders) > 0 {
		...
		for provName, services := range pg.CloudProviders {
			...
			for i, svc := range slices.Collect(maps.Keys(services)) {
				svcStrings[i] = Link(cloudServices[svc])
			}
			builder.WriteString(fmt.Sprintf("- %s\n", Link(providers[provName])))
	...
	if len(pg.RelatedInterests) > 0 {
		...
		for interest, _ := range pg.RelatedInterests {
			builder.WriteString(fmt.Sprintf("- %s\n", linkForInterest(interest))) // TODO: remove interests because of overlap with subject matters???
		}
	}
...
func (p *Project) WithTags(tags ...tags.Tag) *Project {
	p.Tags.Add(tags...)
	return p
}
...
func NewPersonalProject(name string, summary string, link *string, size projectSize) *Project {
	return newProject(name, summary, personalProjectTypeInfo{}, link, size)
}

func NewProfessionalProject(name string, summary string, client *Client, link *string, size projectSize) *Project {
	out := newProject(name, summary, client.ProjectTypeInfo(), link, size)
	client.WithProjects(out)
	return out
}

func NewSchoolProject(name string, summary string, school *SchoolPage, link *string, size projectSize) *Project {
	out := newProject(name, summary, school.ProjectTypeInfo(), link, size)
	school.withProjects(out)
	return out
}

func newProject(name string, summary string, info projectTypeInfo, link *string, size projectSize) *Project {
...
var (
	projectAgentSwarm              = NewPersonalProject("AI Agent Swarm", fixmeLink, nil, projectSizeS)
	polygonBuilderProject          = NewProfessionalProject("Polygon Builder", fixmeLink, jdClient, nil, projectSizeL)
	tileGenProject                 = NewProfessionalProject("Tile Generator", fixmeLink, jdClient, nil, projectSizeL)
	GhaRunnersProject              = NewProfessionalProject("Github Actions GPU Runners", "FIX SUMMARY", jdClient, nil, projectSizeM)
	ogreProject                    = NewProfessionalProject("Organizational Geospatial Rollup Engine", fixmeLink, jdClient, nil, projectSizeL)
	renderProject                  = NewProfessionalProject("Render Cluster", fixmeLink, jdClient, nil, projectSizeL) // TODO: MORE!
	statsProject                   = NewProfessionalProject("Statistics Cluster", fixmeLink, jdClient, nil, projectSizeL)
	billingProject                 = NewProfessionalProject("Billing Cluster", fixmeLink, jdClient, nil, projectSizeL)
	explorerProject                = NewProfessionalProject("Transform Explorer", fixmeLink, jdClient, nil, projectSizeM)
	wqdbProject                    = NewProfessionalProject("Work Queue Database", fixmeLink, jdClient, nil, projectSizeM)
	supportProject                 = NewProfessionalProject("Support Cluster", fixmeLink, jdClient, nil, projectSizeM)
	scudsProject                   = NewProfessionalProject("Scuds API", fixmeLink, jdClient, nil, projectSizeL)
	ufoProject                     = NewProfessionalProject("UFO API", fixmeLink, jdClient, nil, projectSizeL)
	goweProject                    = NewProfessionalProject("Gowe Builder", fixmeLink, jdClient, nil, projectSizeL)
	rasterRenderProject            = NewProfessionalProject("Raster Render", fixmeLink, jdClient, nil, projectSizeL)
	simpsonUnivProject             = NewProfessionalProject("Simpson University", fixmeLink, simpsonUniversityClient, nil, projectSizeS) // TODO: MORE!
	jamfProject                    = NewProfessionalProject("JAMF companywide setup", fixmeLink, sourceAlliesClient, nil, projectSizeM)
	smallImprovementsProject       = NewProfessionalProject("Small Improvements Bot", fixmeLink, sourceAlliesClient, nil, projectSizeS)
	internalResumeGeneratorProject = NewProfessionalProject("Source Allies Internal Consultant Resume Generator", fixmeLink, sourceAlliesClient, nil, projectSizeM)
	mushDbProject                  = NewPersonalProject("MushDb", fixmeLink, &mushDbUrl, projectSizeL)
	cvProject                      = NewPersonalProject("Personal Site and CV", "This project! A generator which creates markdown files that can be viewed via Obsidian, or published to the web.", &cvUrl, projectSizeM) // TODO: make multiple strings an available option for summary
	linksPage                      = NewPersonalProject("Personal Links Page", "A page to put all my links", nil, projectSizeS)                                                                                           // TODO: LINKS PAGE
	measurementsProject            = NewPersonalProject("Measurements", fixmeLink, &measurementsUrl, projectSizeM)
	nfcScannerProject              = NewPersonalProject("Nfc Scanner", fixmeLink, &nfcScannerUrl, projectSizeS)
	teiProjects                    = NewProfessionalProject("Tei Projects", fixmeLink, teiClient, nil, projectSizeS) // TODO: maybe add an actual project
	taePhotoImporter               = NewProfessionalProject("Tae Field Photograph Importer", fixmeLink, taeClient, nil, projectSizeS)
	mafcProjects                   = NewProfessionalProject("Mafc Projects", fixmeLink, mafcClient, nil, projectSizeS) // TODO: maybe add actual projects?
	coreShufflerProject            = NewPersonalProject("Simulate Core Shuffler", fixmeLink, &coreShufflerUrl, projectSizeM)
	CharityProject                 = NewProfessionalProject("Charity Site", fixmeLink, charityClient, nil, projectSizeM)
	WellAwareProject               = NewProfessionalProject("Well Aware NC", fixmeLink, clarkClient, nil, projectSizeM) // TODO: change client to the lab???
	CritColaProject                = NewProfessionalProject("CritCola", fixmeLink, critColaClient, nil, projectSizeM)   // TODO: ADD OTHER CRITCOLA PROJECTS
	ArrowNailProject               = NewProfessionalProject("Hail History Tracker", fixmeLink, arrowNailClient, nil, projectSizeM)
	WildlifeRProject               = NewProfessionalProject("Wildlife R Data Analysis", fixmeLink, wildlifeRClient, nil, projectSizeM)
	MastersDataAnalysisProject     = NewProfessionalProject("Masters Data Analysis", fixmeLink, clarkClient, nil, projectSizeM)
	capstoneProject                = NewSchoolProject("Capstone Project-Uranium Silicide Accident Tolerant Fuel cycle design for Duke Energy Catawba Nuclear Plant", "FIX M_E", schoolNCSU, &coreShufflerUrl, projectSizeL)
	reactorAnalysisFinal           = NewSchoolProject("Reactor Analysis Exam", "Reactor analysis final exam code", schoolNCSU, utils.Pointer(fixmeLink), projectSizeM) // TODO: ADD REAL LINK!
	monteCarloProject              = NewSchoolProject("MonteCarlo nuclear scattering and decay project", "FIX_ME", schoolNCSU, utils.Pointer(fixmeLink), projectSizeM) // TODO: ADD REAL LINK!
	// TODO; FORTRAN SCHOOL PROJECT FOR ATM
	projectLinAlgCryptography = NewSchoolProject("Linear algebra cryptography algorithm", "FIX M_E", schoolNCSU, nil, projectSizeM)
	cherenkovProject          = NewSchoolProject("Cherenkov radiation sensor", fixmeLink, schoolNCSU, nil, projectSizeM)
	roboticsTeamProject       = NewSchoolProject("Robotics team 3720", fixmeLink, schoolCata, nil, projectSizeM)
	cncLaserCutterProject     = NewSchoolProject("CNC Laser Cutter", fixmeLink, schoolCata, nil, projectSizeM)
	aerospaceFinalProject     = NewSchoolProject("Aerospace senior design course", "Designed, created, and tested a rocket from scratch", schoolCata, nil, projectSizeM)
	miscSmallPersonalProjects = NewPersonalProject("Misc small personal projects", "A conglomeration of personal projects which did not each deserve their own entry", nil, projectSizeS)
)

func initProjectsFinal() {
	projectAgentSwarm = projectAgentSwarm.WithStatus(statusMaintaining).
		WithSummary("Create an AI coding agent swarm with "+Link(lookup("Langgraph"))+" and "+Link(lookup("Poolside AI"))).
	...
	polygonBuilderProject = polygonBuilderProject.WithStatus(statusMaintaining).
		WithSummary("Service that utilizes a modified AlphaShapes algorithm to take inputs of millions of datapoints of geospatiotemporal agribusiness data, and output multipolygons representing field operations and subsets thereof.").
	...
	GhaRunnersProject = GhaRunnersProject.WithStatus(statusMaintaining).
		...
		WithSubjectMatters(smBackend, smCiCd, smDevSecOps, smContainerization, smIAC, smCiCd).
		finalize()
	...
	statsProject = statsProject.WithStatus(statusMaintaining).
		...
		WithCaches("Redis", "memcached").
		WithTechnologies("CGo", "Swagger", "OpenAPI", "CUDA", "Github Actions", "Parquet", "Avro", "GraphQL", "SIMD", "REST API").
		WithPlatforms("Datadog", "Logcentral", "Github", "ServiceNow", "GraphQL Apollo", "Azure DevOps", "Jira", "Confluence").
		WithCloudProvider("AWS",
			"ECS", "S3", "EC2", "IAM", "SecretsManager", "DynamoDB", "DAX", "Cloudwatch", "Lambda", "ECR", "Route53", "Kinesis", "SQS", "SNS",
		).
	...
	goweProject = goweProject.WithStatus(statusMaintaining).
		...
		WithLang("Docker", Regularly).
		WithCloudProvider("AWS",
			"ECS", "Fargate", "VPC", "VPN", "S3", "EC2", "IAM", "SecretsManager", "DynamoDB", "DAX", "Cloudwatch", "Lambda", "ECR", "Route53", "Kinesis", "SQS", "SNS", "Kinesis",
		).
	...
	rasterRenderProject = rasterRenderProject. // TODO: this whole thing!
							...
							WithCloudProvider("AWS", "ECS", "EC2", "API Gateway", "IAM", "ELB", "ALB", "Corretto").
							finalize()
	...
	cvProject = cvProject.WithStatus(statusBuilding).
		...
		WithSubjectMatters(smCiCd, smFrontend, smDevSecOps, smServerless).
		finalize()
	linksPage = linksPage.
		WithStatus(statusComplete).
		WithLang("Html", Regularly).
	...
	WellAwareProject = WellAwareProject.WithStatus(statusComplete).
		...
		WithSubjectMatters(smCiCd, smDevSecOps, smFrontend).
		finalize()
	CritColaProject = CritColaProject.WithStatus(statusComplete).
		...
		WithSubjectMatters(smBackend, smCiCd, smDevSecOps, smScripting).
	...
	cherenkovProject = cherenkovProject.WithStatus(statusComplete).
		WithSummary("Designed and built a sensor for Cherenkov radiation, which was tested by lowering the sensor into the core of the PULSTAR reactor at "+Link(schoolNCSU)+". The sensor utilized and Arduino for signal processing.").
		WithLang("Python", Often).
		WithLang("Fortran", Often).
	...
	smallImprovementsProject = smallImprovementsProject.
		...
		WithPlatforms("Slack", "Small Improvements", "Github", "Jira", "Confluence", "Slack").
		WithTechnologies("Github Actions", "REST API").
		WithStatus(statusComplete).
	...
	internalResumeGeneratorProject = internalResumeGeneratorProject.
		...
		finalize()
	// TODO: root-finding algorithms project
	miscSmallPersonalProjects = miscSmallPersonalProjects.
		...
		WithLang("Go", Extensively, smThermodynamics, smScripting, smBackend).
		WithLang("Java", Some, smBackend).
		WithLang("Javascript", Often, smFrontend, smScripting).
		WithLang("Typescript", Often, smFullStack, smScripting).
		WithLang("Kotlin", Rarely, smBackend).
		WithLang("Rust", Rarely, smBackend).
		WithLang("Lua", Rarely, smScripting).
		WithLang("Batch", Rarely, smScripting).
		WithLang("Assembly", Rarely, smEmbeddedSystems).
		WithLang("WASM", Rarely, smFrontend).
		WithLang("ARM Assembly", Rarely, smEmbeddedSystems).
		WithLang("ARM Assembly", Rarely, smEmbeddedSystems).
		WithLang("Zig", Rarely, smBackend).
		WithLang("Cobol", Rarely, smBackend).
		WithLang("LISP", Rarely, smBackend).
		WithLang("LABVIEW", Rarely, smScripting). // TODO: ok or put elsewhere?
		WithLang("MATLAB", Rarely, smScripting).  // TODO: ok or put elsewhere?
		WithLang("Perl", Rarely, smScripting).
		WithLang("PowerShell", Rarely, smScripting).
		WithLang("Ruby", Rarely, smScripting).
		WithTechnologies("Kubernetes", "Kafka", "ORC").
